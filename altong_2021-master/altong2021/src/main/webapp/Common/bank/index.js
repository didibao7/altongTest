...
const generateToggleHandler = function(target) {
  ...
  const handleDetailOpen = function(section) {
    ...
    arrow.innerHTML = '<img src="/pub/member/bank/index/bank_arrow.svg" style="transform:rotate(180deg)">';
  ...
  const handleDetailClose = section => {
    ...
    arrow.innerHTML = '<img src="/pub/member/bank/index/bank_arrow.svg" style="transform:rotate(0deg)">';
    slideUp(detail, 200);
  };
...
const checkTotalIncrease = data => {
  ...
  if (increaseA + increaseB > 0) {
    totalIncrease.parentElement.style.color = "";
...
const handleGraphDetail = e => {
  ...
  //console.log("A : " + arrow.innerHTML)
  const isOpen = arrow.innerHTML == '<img src="/pub/member/bank/index/bank_arrow.svg" style="transform:rotate(180deg)">' ? true : false;
...
const renderDetailHistory = (data, option = true) => {
  ...
  if (data.length == 0) {
    ...
    changeText.innerHTML = getLangStr("jsm_0001");
    changeText.style.fontStyle = "italic";
  } else if (data.length == 20) {
    changeText.style.display = "";
    changeText.innerHTML = getLangStr("jsm_0002");
  } else {
  ...
  const newMarkup = data.reduce((acc, cur, idx) => {
    ...
    return (acc += `<tr class="atm_bank_reTblCon ${rowGray}">
            <td onClick="$(this).html('${cur.regdate.slice(0, 10) + '<br>' + ( (cur.regdate2.split(' ')[4]=='AM'? (cur.regdate2.split(' ')[3].toString().split(':')[0] < 10 ? '0': '' ) : (cur.regdate2.split(' ')[3].toString().split(':')[0] == 12 ? '': 12 )) + parseInt(cur.regdate2.split(' ')[3].toString().split(':')[0]) ) + ':'  + cur.regdate2.split(' ')[3].toString().split(':')[1] + ':'  + cur.regdate2.split(' ')[3].toString().split(':')[2] }')">${cur.regdate.slice(0, 10)}</td>
            <td>${cur.TradeType}</td>
...
const viewAcceptance = data => {
  ...
  if (acceptanceWrapper.length == 0) {
    acceptanceWrapper.innerHTML =
      acceptMarkup +
      `<tr><td id="atm_bank_accText" colspan="2">${getLangStr("jsm_0001")}</td></tr>`;
  } else {
