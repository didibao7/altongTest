...
<%
	...
	String QuestionSeq = request.getAttribute("QuestionSeq").toString();
	pageContext.setAttribute("maxSiren", maxSiren);
	pageContext.setAttribute("QuestionSeq", QuestionSeq);
...
function goTranslate_qr(contentSeq, contentType) {
	var trn_chk;
	if(contentType == 'R') {
...
<c:forEach var="item" items="${questionList}" varStatus="status">
	...
	<div class="replydiv_user_list atm_reply_el" data-seq="${item.seq}">
		...
		<table>
	        ...
	        <tr>
	            <td></td>
	            <td><a href="${replyMemberProfileUrl}">${item.nickName}</a> · <b class="date_format dtFrm" id="qreply_${item.seq}">${item.conDate} <span>${item.conDate}</span></b> 
				<c:if test="${(item.memberSeq eq userSeq or userLv eq '99') and item.memberSeq ne '10003513' }">
				. <i onclick="goConfirm('frm', '/answer/replydel?ReplySeq=${item.seq}&Flag=Q', '삭제', 'answerList_ifrm');"><spring:message code="msg_0228"/></i>
				</c:if>
                ...
                <div class="reply_lang_icon"><img class="answer_lang" src="/Common/images/language.svg" alt='<spring:message code="msg_0233"/>' id="qrT_ai_${item.seq}" onclick="goTranslate_qr('${item.seq}', 'R');"></div>
                </c:if>
				</td>
	         </tr>
		</table>
	</div>
	<script>formatDateForSpan('qreply_${item.seq}');</script>
</c:forEach>	