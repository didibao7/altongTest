...
<script>
	...
	function viewCateoriedQuestion() {
		...
		$.ajax({
			url: './answerView/GetCategoryCodeName?' + formParameter +  '&time=' + new Date(),
			type: 'get',
			async: false,
			success: function(data) {
				categoryCodeName = $.parseJSON(data)[0];
			},
			error: function() {
		...
		$.ajax({
			url: './answerView/GetQuestion?' + formParameter + 'pg=' + page + '&page_size=' + pageSize + '&src_Sort=<%=request.getParameter("src_Sort")%>' + '&time=' + new Date(),
			type: 'get',
			success: function(data) {
				...
				$(window).scrollTop(0);
				history.replaceState(null, null, '/answer/answerView/QuestionList?' + formParameter + 'pg=' + page + '&src_Sort=<%=request.getParameter("src_Sort")%>');
			}
		});
	}
