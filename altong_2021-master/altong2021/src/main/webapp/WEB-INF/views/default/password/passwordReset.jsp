...
<!-- 210527 비밀번호 찾기 나라별 국기 스타일 추가 -->
<link rel="stylesheet" href="/Common/CSS_new/dd.css?ver=1.4">
<script src="/Common/js_new/default/jquery.dd.min.js?ver=1.4"></script>
<script>
    $(document).ready(function (){
        try {
            $('.atm_edittop_select').msDropDown();
        } catch (e) {
            alert(e.message);
        }
    });
...
<div id="passwordReset_wrapper">
	<div class="center">
		...
		<div class="contentsWrapper" align="center">
			<p id="title"><spring:message code="msg_0389"/></p>
			<form name="pw_frm_sch" method="post" onSubmit="return false;">
				<div>
					<select name="country" class="atm_edittop_select" onchange="$('input[name=phone]').select()">
					...
					<input name="phone" type="tel" pattern="[0-9]{11}" placeholder='<spring:message code="msg_0391"/>' onKeyUp="checkNum(this);">
				</div>
			</form>
			<p class="sendBtn" onClick=""><spring:message code="msg_0390"/></p>
		</div>
	</div>
</div>
<script src="/pub/default/passwordReset/passwordReset.js?ver=1.1"></script>
</body>