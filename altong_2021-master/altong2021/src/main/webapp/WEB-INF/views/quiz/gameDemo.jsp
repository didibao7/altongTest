	...
	if(distance > 0) {
		...
		if(view == 'Y') {
			if(parseInt(next) <= parseInt(max)) {
				location.href = '/quiz/gameDemo?uid=' + uid + '&ord=' + next + '&view=T';
			}
			else {
				$('#counter0').css('display', 'none');
				$('#counter1').css('display', 'none');
			...
			if(parseInt(next) <= parseInt(max)) {
				if(success != 'V') {
					if(confirm('계속 볼래요?')) {
						location.href = '/quiz/gameDemo?uid=' + uid + '&ord=' + next + '&view=V';
					}
					else {
						//중단
						quizStop();
...
function quizQuit() {
	if(userLv != '99') {
		...
		location.href = '/quiz/demoEnd?uid=' + uid + '&current=' + current + '&max=' + max;
	}
}
</script>
</head>
