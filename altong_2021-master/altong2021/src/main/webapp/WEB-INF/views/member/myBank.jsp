...
<%@ include file="/Common/include/MenuSub.jsp" %>
	...
	<div class="atm_bank_myaccount_pcgray">
		<div class="atm_bank_myaccount" align="center">
			<p class="atm_bank_c1"><spring:message code="msg_0804"/> <span class="atm_color_org atm_superbold"><fmt:formatNumber value="${UserAlmoney}" pattern="#,###.#" /></span><spring:message code="msg_0136"/></p>
		    <div class="atm_bank_line"></div>
			<p class="atm_bank_c2"><span class="atm_whitespace"><spring:message code="msg_0805"/> <span class="atm_color_min atm_superbold"><fmt:formatNumber value="${Import}" pattern="#,###.#" /></span><spring:message code="msg_0136"/></span> <span class="atm_bank_color_gray">&nbsp;|&nbsp;</span> <span class="atm_whitespace"><spring:message code="msg_0806"/> <span class="atm_color_org atm_superbold"><fmt:formatNumber value="${Expense}" pattern="#,###.#" /></span><spring:message code="msg_0136"/></span></p>
			<p class="atm_bank_c3a" onClick="alert('준비중입니다.');">대출</p>
			<p class="atm_bank_c3b" onClick="alert('준비중입니다.');">결제</p>
			<p class="atm_bank_c3c" onClick="alert('준비중입니다.');">이체</p>
			
			<c:choose>
				<c:when test="${userLv < 2}"><p class="atm_bank_c3d" onClick="alert('\'나비천사\' 이상의 등급부터 사용할 수 있습니다.');">환전</p></c:when>
				<c:otherwise><p class="atm_bank_c3d" onClick="location.href='/member/account/exchange/exchangeAsk';">환전</p></c:otherwise>
			</c:choose>
	...
	<div class="atm_ranka_tab1">
		<div class="atm_ranka_tab1_pc">
			<p class="atm_ranka_tabc1<c:if test="${flagBank == 'I'}">_on</c:if>" onClick="javascript:location.href='/member/myBank?FlagBank=I';">수입 / 지출 내역</p>
			<p class="atm_ranka_tabc2">|</p>
			<!--<p class="atm_ranka_tabc3<c:if test="${flagBank == 'E'}">_on</c:if>" onClick="javascript:location.href='/member/myBank?FlagBank=E';">지출내역</p>-->
		</div>
	</div><!-- atm_ranka_tab1 end -->
		...
		<div id="divBody" style="display:none">
			...
			<div class="atm_bank_graph_wrapper" align="center">
				<div class="atm_bank_graph">
					<div class="atm_bank_income" id="BankReport" style="display:none"></div>
				</div>
			</div>
			
			<div class="atm_bank_listwrapper" align="center">
				<div class="atm_bank_list" id="BankList" style="display:none"></div>
			</div>
...
<script>
	function viewMonthData(flag) {
		...
		if(flag != 4) {
			...
			$('input[name=SearchDateS]').val(getFormatDate(startDate))
			$('input[name=SearchDateE]').val(getFormatDate(new Date))
		}
	...
	function viewBankReport() {
		return;
		$.ajax({
			url : "/member/getBankReport",
			data: $('form[name=frm_sch]').serialize(),
			success: function(data) {
				$('#divBody').fadeIn();
				$('#BankReport').html(data).slideDown(1000);
				//fShowBody();
			}
		});
	}
	function viewBankList(page) {
		...
		$.ajax({
			...
			success: function(data) {
				...
				//console.log('count : ' + parseInt(count, 10));
				count = parseInt(count, 10);
				$('#divBody').fadeIn();
				$('#BankList').html(html).fadeIn(1000);
				//fShowBody();
