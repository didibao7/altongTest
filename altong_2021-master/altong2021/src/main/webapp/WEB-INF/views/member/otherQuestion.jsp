...
<%@ include file="/Common/include/MenuSub.jsp" %>
	...
	<div class="atm_mycatebar1">
		<div class="atm_mycatebar1_pc">
			<p class="atm_mycatebar_c1">작성한 질문 <span class="atm_mycatebar_c2">${n_trec}</span> 개</p>
	...
	<c:if test="${que.flagNickName != 'N'}">
		<div class="atm_myjjim_el">
			<div class="atm_myjjim_c5" onClick="javascript:location.href='/answer/answerList?Seq=${que.questionSeq}&CurPageName=/member/otherQuestion';"><img src="/Common/images/icon_question.png" class="atm_icon_qmark"/><p class="atm_icon_score"><fmt:formatNumber value="${que.almoney}" pattern="#,###.0" /></p>${que.title}</div><br />
			<p class="beefup__head" onClick="javascript:location.href='/answer/answerList?Seq=${que.questionSeq}&CurPageName=/member/otherQuestion';">${que.contents.replaceAll("<(/)?([a-zA-Z]*)(\\s[a-zA-Z]*=[^>]*)?(\\s)*(/)?>","")}</p>
			<p class="atm_myjjim_c7">${que.dateReg} &nbsp;
				<span class="atm_whitespace"><img src="/Common/images/icon_view.png" class="atm_viewicon"/><fmt:formatNumber value="${que.readCount}" pattern="#,###" /></span>&nbsp;
				<span class="atm_whitespace"><img src="/Common/images/icon_answer_micro.png" class="atm_viewicon"/>${que.answerCount}</span>
			</p>
		</div>
	</c:if>
	</c:forEach>
