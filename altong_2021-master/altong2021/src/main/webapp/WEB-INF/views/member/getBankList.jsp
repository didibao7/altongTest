...
<c:if test="${myBankCnt != '0'}">
	...
	<c:choose>
		...
		<!-- 수입 / 지출  -->
	<c:choose>
		<c:when test="${bnk.almoney < 0 || bnk.tradeType == 'View' || bnk.tradeType == 'Question' || bnk.tradeType == 'Exchange' || bnk.tradeType == '1' || bnk.tradeType == '2' || bnk.tradeType == '4' || bnk.tradeType == 'ExchangeAlpay'}">
		<td class="atm_bank_mylist2" style="color: #f00; text-align: right; padding-right:5px;">- <c:if test="${bnk.almoney < 0}"><c:set var="minusAlmoney" value="${bnk.almoney * -1}"/></c:if><fmt:formatNumber value="${minusAlmoney}" pattern="#,###.#" /></td>
		</c:when>
		<c:otherwise>
		<td class="atm_bank_mylist2" style="color: #00f; text-align: right; padding-right:5px;">+ <fmt:formatNumber value="${bnk.almoney}" pattern="#,###.#" /></td>
		</c:otherwise>
	</c:choose>
